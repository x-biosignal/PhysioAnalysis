...
epochSliding <- function(x, window, step = NULL, baseline = NULL) {
  ...
  # Use epochData internally (suppress its provenance; epochSliding records
  # the single top-level activity below)
  result <- .suppressProvenance(
    epochData(x, tmin = 0, tmax = window,
  ...
  .recordProv(result, input_assay = defaultAssay(x),
              output_assay = "metadata:sliding_window", .package = "PhysioAnalysis")
}
