...
#' @return A new `PhysioExperiment` object with a 4D assay
...
#' @references Luck, S.J. (2014). "An Introduction to the Event-Related
#'   Potential Technique." 2nd ed. MIT Press.
#' @seealso [averageEpochs()] to average across epochs, [epochTimes()] to get
...
epochData <- function(x, tmin = -0.2, tmax = 0.8, event_type = NULL,
  ...
  # Determine number of channels and samples based on dimensionality. The 2D
  # matrix is left un-reshaped here: its column-major layout is identical to a
  # (time x channels x 1) array, and the fixed-length kernel consumes it
  # directly. Only the variable-length path (below) needs an explicit 3D view.
  if (length(dims) == 2) {
    n_channels <- dims[2]
    n_samples <- 1L
  ...
  if (variable_length_mode && !is.null(variable_lengths)) {
    # Variable-length extraction needs the NA-padded array and a 3D data view.
    if (length(dims) == 2) {
      data <- array(data, dim = c(dims[1], dims[2], 1))
    }
    epoched <- array(NA_real_, dim = c(epoch_length, n_channels, n_epochs,
                                       n_samples))
    ...
    # Standard fixed-length extraction via the compiled kernel (contiguous
    # memcpy per epoch/channel/sample; the events are already in-bounds). `data`
    # is passed as-is (2D matrix or 3D array; column-major layout matches),
    # avoiding a reshape/flatten copy. The pure-R gather is retained as
    # .epochFixedR() for numerical-parity tests.
    epoched <- cpp_epoch_fixed(data, n_timepoints, n_channels,
                               n_samples, as.integer(valid_samples),
                               pre_samples, post_samples)
    dim(epoched) <- c(epoch_length, n_channels, n_epochs, n_samples)
  }
  ...
  .recordProv(new_pe, input_assay = assay_name, output_assay = "epoched",
              .package = "PhysioAnalysis", from = x)
}

# Pure-R fixed-length epoch extraction (pre-0.3.4 gather), retained as the
# numerical-parity reference for cpp_epoch_fixed. `data` is the (time x channel
# x sample) 3D array; `centers` are 1-based event sample positions.
.epochFixedR <- function(data, centers, pre_samples, post_samples) {
  dims <- dim(data)
  epoch_length <- pre_samples + post_samples + 1L
  n_channels <- dims[2]; n_samples <- dims[3]; n_epochs <- length(centers)
  epoched <- array(NA_real_, dim = c(epoch_length, n_channels, n_epochs,
                                     n_samples))
  time_offsets <- seq(-pre_samples, post_samples)
  idx_matrix <- outer(time_offsets, centers, FUN = function(t, s) t + s)
  for (ch in seq_len(n_channels)) {
    for (s in seq_len(n_samples)) {
...
#' @return A `PhysioExperiment` object with an `"averaged"` assay containing
#'   the mean signal across epochs. If `by` is specified, the third dimension
#'   corresponds to conditions.
#' @references Luck, S.J. (2014). "An Introduction to the Event-Related
#'   Potential Technique." 2nd ed. MIT Press.
#' @seealso [epochData()] to create epoched data, [grandAverage()] for
#'   averaging across subjects, [plotERP()] for ERP visualization.
#' @export
averageEpochs <- function(x, by = NULL) {
  ...
  .recordProv(new_pe, input_assay = assay_name, output_assay = "averaged",
              .package = "PhysioAnalysis", from = x)
}
...
#' @return A `PhysioExperiment` object with a `"grand_average"` assay
#'   containing the mean across all input objects. Metadata includes
#'   `n_subjects` indicating the number of objects averaged.
#' @references Luck, S.J. (2014). "An Introduction to the Event-Related
#'   Potential Technique." 2nd ed. MIT Press.
#' @seealso [averageEpochs()] for within-subject averaging, [epochData()] to
#'   create epoched data, [plotERP()] for ERP visualization.
#' @export
grandAverage <- function(...) {
  ...
  .recordProv(
    ...
    input_assay = defaultAssay(ref), output_assay = "grand_average",
    .package = "PhysioAnalysis", from = ref
  )
}
...
#'
#' @param x An epoched PhysioExperiment object.
#' @return Numeric vector of times in seconds relative to event onset
#'   (negative values represent pre-stimulus time).
#' @references Luck, S.J. (2014). "An Introduction to the Event-Related
#'   Potential Technique." 2nd ed. MIT Press.
#' @seealso [epochData()] which creates the epoched data, [averageEpochs()]
#'   for epoch averaging, [plotERP()] for ERP visualization.
#' @export
