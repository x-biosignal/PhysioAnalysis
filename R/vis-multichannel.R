...
#'   Analysis}. Springer-Verlag New York. \doi{10.1007/978-3-319-24277-4}
#'
#'   Delorme, A. & Makeig, S. (2004). "EEGLAB: an open source toolbox for
#'   analysis of single-trial EEG dynamics including independent component
#'   analysis." \emph{Journal of Neuroscience Methods}, 134(1), 9-21.
#'   \doi{10.1016/j.jneumeth.2003.10.009}
#' @seealso [plotSignal()] for single-channel plots, [plotERP()] for
...
plotMultiChannel <- function(x, channels = NULL, sample = 1L,
  ...
  # default to the colorblind-safe ecosystem palette when no colours are given.
  # For >8 channels physioPalette() warns that qualitative colours are no longer
  # CVD-distinguishable; that guidance ("facet / use a sequential encoding") does
  # not apply to signal traces (channels are also separated by the legend and, in
  # the stacked style, by vertical offset), so the warning is suppressed here.
  if (is.null(colors)) {
    colors <- suppressWarnings(PhysioCore::physioPalette(n_channels))
  }
  if (length(colors) >= n_channels) {
...
#'   analysis." \emph{Journal of Neuroscience Methods}, 134(1), 9-21.
#'   \doi{10.1016/j.jneumeth.2003.10.009}
#' @seealso [plotMultiChannel()] for multi-channel visualization,
#'   [plotTopomap()] for topographic maps, [epochData()] for creating
#'   epoched data, [bootstrapCI()] for bootstrap confidence intervals.
#' @export
...
#'   analysis." \emph{Journal of Neuroscience Methods}, 134(1), 9-21.
#'   \doi{10.1016/j.jneumeth.2003.10.009}
#' @seealso [plotSignal()] for time-domain signal plots, [fftSignals()] for
#'   FFT computation, [bandPower()] for frequency band power extraction,
#'   [plotSpectrogram()] for time-frequency visualization.
#' @export
