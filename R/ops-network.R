...
# Guard the graph-metric entry points: the C++ backends index adjacency(i, j)
# over n = nrow, so a non-square matrix (e.g. raw time x channel data passed by
# mistake) reads out of bounds and aborts the whole R session. Fail with a clear
# error instead.
.assert_square_adjacency <- function(adjacency, arg = "adjacency") {
  if (!is.matrix(adjacency) || nrow(adjacency) != ncol(adjacency)) {
    stop(sprintf("`%s` must be a square (n x n) adjacency matrix; got %s.",
                 arg, if (is.matrix(adjacency))
                        paste(dim(adjacency), collapse = " x ")
                      else class(adjacency)[1]),
         call. = FALSE)
  }
}
...
#' @param use_cpp If TRUE (default), use the compiled C++ backend, falling back
#'   to R if unavailable.
...
nodeDegree <- function(adjacency, weighted = FALSE, use_cpp = TRUE) {
  .assert_square_adjacency(adjacency)
  diag(adjacency) <- 0
...
#' @param use_cpp If TRUE (default), use the compiled C++ backend, falling back
#'   to R if unavailable.
#' @param n_cores Number of OpenMP threads for the C++ backend (default 1).
...
slidingWindowConnectivity <- function(x, window_size = 256L, step = 64L,
  ...
  for (w in seq_len(n_windows)) {
    ...
    if (method == "correlation") {
      # correlationMatrix() returns a bare channel x channel matrix.
      conn_matrices[, , w] <- correlationMatrix(pe_window, assay_name = "window", ...)
