...
# --- WSF-03: non-square input must error cleanly, not abort the R session ---
test_that("graph metrics reject non-square input instead of crashing C++", {
  bad <- matrix(rnorm(200 * 8), nrow = 200, ncol = 8)   # raw data, not adjacency
  expect_error(clusteringCoefficient(bad), "square")
  expect_error(betweennessCentrality(bad), "square")
  expect_error(nodeDegree(bad), "square")
  expect_error(eigenvectorCentrality(bad), "square")
  expect_error(globalEfficiency(bad), "square")
  expect_error(pathLength(bad), "square")
})
